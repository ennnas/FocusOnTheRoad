...
def parse_img(model, oriImg):
    ...
    for m in range(len(multiplier)):
        ...
        im = (
            np.transpose(np.float32(imageToTest_padded[:, :, :, np.newaxis]), (3, 2, 0, 1)) / 256
            - 0.5
        )
        ...
        heatmap = heatmap[
            : imageToTest_padded.shape[0] - pad[2], : imageToTest_padded.shape[1] - pad[3], :
        ]
        heatmap = cv2.resize(
    ...
    for part in range(18):  # ignore background part
        map_ori = heatmap_avg[:, :, part]  # confidence map of a single body part
    ...
    for k in range(len(mapIdx)):  # connect all 18 body parts
        score_mid = paf_avg[
            :, :, [x - 19 for x in mapIdx[k]]
        ]  # get the 2 associated pafs maps for that body part
        candA = all_peaks[limbSeq[k][0] - 1]  # get the possible peaks for the associated bodypart
        ...
        if nA != 0 and nB != 0:
            ...
            for i in range(nA):
                for j in range(nB):
                    vec = np.subtract(
                        candB[j][:2], candA[i][:2]
                    )  # get the limb as the difference between the two position vector of the body parts
                    norm = (
                        math.sqrt(vec[0] * vec[0] + vec[1] * vec[1]) + 1e-15
                    )  # just to avoid DivisionByZero
            ...
            for i in range(len(connection_all[k])):  # = 1:size(temp,1)
                ...
                if found == 1:
                    ...
                    if subset[j][indexB] != partBs[i]:
                        ...
                        subset[j][-1] += 1
                        subset[j][-2] += (
                            candidate[partBs[i].astype(int), 2] + connection_all[k][i][2]
                        )
                elif found == 2:  # if found 2 and disjoint, merge them
                    j1, j2 = subset_idx
                    membership = ((subset[j1] >= 0).astype(int) + (subset[j2] >= 0).astype(int))[
                        :-2
                    ]
