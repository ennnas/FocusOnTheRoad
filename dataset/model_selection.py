...
def train_test_split(
    ...
    sampled_df = (
        grouped_df.groupby(group_ids).apply(lambda x: x.sample(num_images)).reset_index(drop=True)
    )
    test_df = (
        sampled_df.groupby(group_ids).apply(lambda x: x.sample(val_images)).reset_index(drop=True)
    )
